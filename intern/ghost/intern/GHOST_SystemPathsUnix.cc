...
const char *GHOST_SystemPathsUnix::getSystemDir(int /*version*/, const char *versionstr) const
{
  ...
  if (static_path) {
    static string system_path = string(static_path) + "/xfblender/" + versionstr;
...
const char *GHOST_SystemPathsUnix::getUserDir(int version, const char *versionstr) const
{
  ...
  if (version < 264) {
    if (user_path.empty() || last_version != version) {
      ...
      if (home) {
        user_path = string(home) + "/.xfblender/" + versionstr;
      }
      else {
        return nullptr;
      }
  ...
  if (user_path.empty() || last_version != version) {
    ...
    if (home) {
      ...
      home = home_dir_get();
      if (home) {
        user_path = string(home) + "/.config/xfblender/" + versionstr;
      }
      else {
        return nullptr;
      }
